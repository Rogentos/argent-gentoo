...
# $Header: /var/cvsroot/gentoo-x86/sci-geosciences/gpsd/gpsd-2.95-r1.ebuild,v 1.4 2011/04/11 07:02:09 xmw Exp $
...
src_configure() {
	...
	sed -i 's|--prefix=${prefix} # --root=$(DESTDIR)|--prefix=${prefix} --root=$(DESTDIR)|' Makefile.am Makefile.in || die
	sed -i 's|/usr/bin/python|/usr/bin/python2|' gpsd.hotplug{,.wrapper} || die
	sed -i 's|/usr/bin/python2.6|/usr/bin/python2|' valgrind-audit || die
	sed -i 's|/usr/bin/env python|/usr/bin/env python2|' gpscat \
		gpsfake gpsprof xgps xgpsspeed gps/gps.py || die
	...
	PYTHON=/usr/bin/python2 econf WITH_XMLTO=yes WITH_XSLTPROC=no \
		--build=${CBUILD} \
