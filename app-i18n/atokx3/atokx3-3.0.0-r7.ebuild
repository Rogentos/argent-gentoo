	...
	if use amd64 && has_multilib_profile ; then
		if [ -x "${EPREFIX}"/usr/bin/gtk-query-immodules-2.0 ] ; then
			gtk_query_immodules_2 "${EPREFIX}"/usr/bin/gtk-query-immodules-2.0 \
				"${EPREFIX}/etc/gtk-2.0/$(get_abi_CHOST)/gtk.immodules"
		fi
		if [ -x "${EPREFIX}"/usr/bin/gtk-query-immodules-2.0-32 ] ; then
			gtk_query_immodules_2 "${EPREFIX}"/usr/bin/gtk-query-immodules-2.0-32 \
				"${EPREFIX}/etc/gtk-2.0/$(get_abi_CHOST x86)/gtk.immodules"
		fi
	else
		if [ -x "${EPREFIX}"/usr/bin/gtk-query-immodules-2.0 ] ; then
			gtk_query_immodules_2 "${EPREFIX}"/usr/bin/gtk-query-immodules-2.0 \
				"${EPREFIX}/etc/gtk-2.0/gtk.immodules"
		fi
	fi
}
